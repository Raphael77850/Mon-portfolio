 ...
 */
function parseVideoUrl(url: string): { type: 'youtube' | 'vimeo' | 'local'; id: string } {
  // YouTube patterns
              ...
              <div className="relative aspect-video bg-black rounded-lg shadow-2xl overflow-hidden">
                ...
                {videoData.type === 'local' && (
                  ...
                  <video
                    ...
                    autoPlay
                    preload="metadata"
                    poster={videoData.id.replace(/\.(mp4|webm)$/, '.jpg').replace('/videos/', '/videos/posters/')}
                    className="absolute inset-0 w-full h-full object-contain"
                    aria-label={`Démo vidéo de ${title}`}
                  >
                    {/* WebM (meilleure compression) pour navigateurs modernes */}
                    <source src={videoData.id.replace('.mp4', '.webm')} type="video/webm" />
